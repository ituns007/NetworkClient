    ...
    public Request okhttp() {
        ...
        try {
            ...
            if(networkMethod == null) {
                ...
                builder.post(RequestBody.create(JSON, networkRequest.body()));
            } else if(networkMethod == NetworkMethod.HEAD) {
                builder.head();
            } else if(networkMethod == NetworkMethod.DELETE) {
                builder.delete(RequestBody.create(JSON, networkRequest.body()));
            } else if(networkMethod == NetworkMethod.PATCH) {
                builder.patch(RequestBody.create(JSON, networkRequest.body()));
            } else if(networkMethod == NetworkMethod.PUT) {
                builder.put(RequestBody.create(JSON, networkRequest.body()));
            }
